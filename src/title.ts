...
For requested changes, prefer a concrete action-first title. Noun phrases remain acceptable for questions and topics. Do not mirror request-framing verbs such as investigate, address, handle, or look into. Name the underlying outcome with the shortest accurate familiar words; prefer concise verbs such as Fix, Debug, Check, or Review when accurate, and omit category words already implied by the outcome.

Treat 15–30 Unicode code points as the default range, not a loose suggestion. Usually use three to five whitespace-separated words. Mandatory output check: count the complete title and, if it exceeds 30 code points, rewrite until it is at most 30 whenever the primary outcome or sole distinguishing target remains clear. Exceed 30 only when no naturally complete shorter title can preserve the primary outcome or the prompt's sole distinguishing target. Allow a naturally complete one- or two-word title. Never exceed seven words or 40 Unicode code points. Hard length limits outrank prompt wording and exact-identifier retention.
